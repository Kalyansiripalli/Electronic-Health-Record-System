...
class UserRegistrationSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = ['email', 'name', 'password', 'ConfirmPassword', 'role', 'is_admin', 'hospitalId']
        extra_kwargs = {
    ...
    def validate(self, attrs):
        ...
        if role != 'doctor' and role != 'admin':
            raise serializers.ValidationError("role must be either admin or doctor")
        if role == 'doctor' and 'hospitalId' not in attrs:
            raise serializers.ValidationError("Hospital ID is required for doctor role")
        if role == 'doctor':
            hospital_id = attrs.get('hospitalId')
            if not HospitalList.objects.filter(id=hospital_id).exists():
                raise serializers.ValidationError("Currently we don't offer services in this hospital. Please reach out to the admin to add this hospital.")
    ...
    def create(self, validated_data):
        ...
        user = User.objects.create_user(**validated_data,email=email, role=role)
        ...
        # Create Doctor entry
        if role == 'doctor':
            DoctorHospitalMapping.objects.create(userid=user.id, hospital_id=hospital_id)
